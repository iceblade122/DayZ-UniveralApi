class UniversalApi extends Managed 
{
	...
	static int Post(string url)
	{
		...
		ctx.POST(m_StaticUApiSilentCallBack, "", "{}");
		return 0;
	}
	...
	//A super simple Post Interface to help people
	static int Post(string url, string jsonString, UApiCallbackBase cb, string contentType = "application/json")
	{
		int cid = UApi().CallId();
		if (cb)
		{
	...
	{
		RestContext ctx =  RestCore().GetRestContext(url);
		ctx.GET(m_StaticUApiSilentCallBack, "");
		return 0;
	}
	...
	//A super simple Get Interface to help people
	static int Get(string url, UApiCallbackBase cb)
	{
		int cid = UApi().CallId();
		if (cb)
		{
	...
	string GetAuthToken()
	{
		if (m_authToken && !GetGame().IsDedicatedServer())
		{
			return m_authToken.GetAuthToken();
		} else if (GetGame().IsDedicatedServer() && UApiConfig().ServerAuth != "")
		{
	...
	protected void RPCUniversalApiConfig(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
	...
	void AddPlayerAuth(string guid, string auth)
	{
		if (!PlayerAuths)
		{
			PlayerAuths = new map<string, string>;
		}
	...
	protected void RPCRequestQnAConfig(CallType type, ParamsReadContext ctx, PlayerIdentity sender, Object target)
	{
		...
		if (!GetGame().IsDedicatedServer() && !PlayerIdentity.Cast(sender) && data.param1)
		{
			m_QnAMakerServerAnswers = data.param1;
		} else if (GetGame().IsDedicatedServer() && PlayerIdentity.Cast(sender) && data.param1 == NULL)
		{
	...
	protected void SendAuthToken(PlayerIdentity identity, string auth)
	{
		if (identity && auth != "")
		{
			Print("[UAPI] Sending PlayerAuth Token to " + identity.GetId());
			UniversalApiConfig clientConfig = new UniversalApiConfig;
			clientConfig.ConfigVersion = UApiConfig().ConfigVersion;
			clientConfig.ServerURL = UApiConfig().ServerURL;
			clientConfig.ServerID = UApiConfig().ServerID;
			clientConfig.ServerAuth = "null";
			clientConfig.QnAEnabled = UApiConfig().QnAEnabled;
			clientConfig.EnableBuiltinLogging = UApiConfig().EnableBuiltinLogging;
			clientConfig.PromptDiscordOnConnect = UApiConfig().PromptDiscordOnConnect;

			ApiAuthToken authToken = new ApiAuthToken;
			authToken.GUID = identity.GetId();
			authToken.AUTH = auth;

			GetRPCManager().SendRPC("UAPI", "RPCUniversalApiConfig", new Param2<ApiAuthToken, UniversalApiConfig>(authToken, clientConfig), true, identity);
		} else 
		{
			Print("[UAPI] [UApiAuthCallBack] ERROR ");
			if (identity)
			{
				UApi().AuthError(identity.GetId());
			}
		}
	}
